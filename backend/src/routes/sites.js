...
router.post('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'INSERT INTO sites (nome) VALUES ($1) RETURNING id, nome, attivo',
      [nome]
    );
    res.status(201).json(rows[0]);
  } catch (err) {
    if (err.code === '23505') {
      return res.status(409).json({ error: 'Sito già presente' });
    }
    throw err;
  }
});
...
router.put('/:id', async (req, res) => {
  ...
  if (!nome) return res.status(400).json({ error: 'Nome obbligatorio' });
  const { rows } = await pool.query(
    'UPDATE sites SET nome = $1, attivo = $2 WHERE id = $3 RETURNING id, nome, attivo',
